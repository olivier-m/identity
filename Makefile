...
source-lint:
	$(ESLINT) --ext .js --ext .vue ./src ./webpack.config.js
...
moz-lint:
	...
	$(WEB_EXT) lint -s $(ADDON)

moz-build: source-lint source-build moz-lint
	unset WEB_EXT_FIREFOX_BINARY WEB_EXT_API_KEY WEB_EXT_API_SECRET ;\
	$(WEB_EXT) build -s $(ADDON) --overwrite-dest
